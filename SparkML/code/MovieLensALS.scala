...
object MovieLensALS {
  def main(args: Array[String]) {
    ...
    val myRatings = loadRatings("ml-latest-small/ratings.csv")
    val myRatingsRDD = sc.parallelize(myRatings, 1)

    // load ratings and movie titles
    val movieLensHomeDir = "ml-latest-small" //args(0)
    ...
    println("candidates: " +  candidates.count())
    
    println("-- top 10 movie candidates for prediction ")
    candidates.collect.take(10).foreach(println)
    //println("bestModel: " +  bestModel.mkString("|"))
    ...
    println("recommendations.length: " +  recommendations.length)
